...
export class AlertsService {
  ...
  async evaluateThresholds(sensorData: SensorData, user?: User): Promise<void> {
    ...
    if (
      ph &&
      ph.value !== undefined &&
      (ph.value < thresholds.ph.min || ph.value > thresholds.ph.max)
    ) {
      alerts.push({
    ...
    if (
      ...
      (temperature.value < thresholds.temperature.min ||
        temperature.value > thresholds.temperature.max)
    ) {
      alerts.push({
    ...
    if (
      tds &&
      tds.value !== undefined &&
      (tds.value < thresholds.tds.min || tds.value > thresholds.tds.max)
    ) {
      alerts.push({
    ...
    if (
      do_level &&
      do_level.value !== undefined &&
      (do_level.value < thresholds.do_level.min ||
        do_level.value > thresholds.do_level.max)
    ) {
      alerts.push({
    ...
    for (const alert of alerts) {
      this.logger.warn(
        `CRITICAL ALERT for device ${device_id}: ${alert.description}`,
      );

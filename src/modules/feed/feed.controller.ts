...
export class FeedController {
  ...
  @ApiResponse({
    status: 201,
    description: 'The feed has been successfully registered.',
  })
  ...
  @Roles(UserRole.USER, UserRole.ADMIN, UserRole.SUPERUSER)
  @ApiOperation({
    summary: 'Get feed analytics for a device',
    description: 'Retrieves feed analytics including FCR, consumption data, and summary for a specific device. Optional date range can be provided.'
  })
  @ApiResponse({
    status: 200,
    description: 'Feed analytics data retrieved successfully',
    type: FeedAnalyticsResponseDto,
  })
  @ApiResponse({
    status: 404,
    description: 'Insufficient data to calculate feed analytics',
  })
  getAnalytics(
    @CurrentUser() user: User,
    @Param('deviceId') deviceId: string,
    ...
    return this.feedService.getAnalytics(
      user,
      deviceId,
      startDate ? new Date(startDate) : undefined,
      endDate ? new Date(endDate) : undefined

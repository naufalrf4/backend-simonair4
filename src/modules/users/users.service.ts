...
export class UsersService {
  ...
  async findAll(queryDto: UserQueryDto): Promise<PaginatedUsers> {
    ...
    if (created_at_from) {
      queryBuilder.andWhere('user.created_at >= :created_at_from', {
        created_at_from,
      });
    }

    if (created_at_to) {
      queryBuilder.andWhere('user.created_at <= :created_at_to', {
        created_at_to,
      });
    }

    if (last_login_from) {
      queryBuilder.andWhere('user.last_login >= :last_login_from', {
        last_login_from,
      });
    }

    if (last_login_to) {
      queryBuilder.andWhere('user.last_login <= :last_login_to', {
        last_login_to,
      });
    }
  ...
  async changePassword(
    ...
    if (!user.password_hash) {
      throw new BadRequestException(
        'Cannot change password for social login users',
      );
    }
  ...
  async remove(id: string): Promise<void> {
    ...
    if (user.devices && user.devices.length > 0) {
      throw new BadRequestException(
        'Cannot delete user with associated devices',
      );
    }
